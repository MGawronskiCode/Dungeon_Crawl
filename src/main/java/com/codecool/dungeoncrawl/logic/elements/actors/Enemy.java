...
public abstract class Enemy extends Actor {
  ...
  protected boolean isValidMove(Cell nextCell, CellType nextCellType) {
    boolean notOutOfMap = checkIfOutOfMap();
    return (notOutOfMap && (nextCellType == CellType.EMPTY || nextCellType == CellType.FLOOR) && nextCell.getActor() == null);
  }

  private boolean checkIfOutOfMap() {
    return true;//todo check if the move is not leading out of map
  }
